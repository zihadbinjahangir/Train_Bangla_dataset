...
class CocoCaption(Dataset):
    def __init__(self, root, ann, max_length, limit, transform=train_transform, mode='training'):
        ...
        self.annot = [(self._process(val['filename']), val['caption'])  ## changed image_id to filename because our json file is like that.
                      for val in ann] ## changed ann['annotations'] to onlay ann because our json file items are inside a list
        if mode == 'validation':
    ...
    def _process(self, image_id):
        val = str(image_id) ## Removed .zfill(12)
        return val ## Removed + '.jpg'
